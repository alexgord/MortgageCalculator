...
def generate_report(report_type: str, output_report_file_name: Path, results: list[MortgageResult], cfg: PropertiesListConfig) -> None:
    ...
    # Generate report content using StringIO buffer
    with io.StringIO() as buffer:
        ...
        for i, row in enumerate(results, 1):
            ...
            cash_to_close_df = pd.DataFrame({
                ...
                "Amount": [f"${cfg.loan_parameters.down_payment:,.2f}", f"${row[K.TOTAL_ONE_TIME_COSTS]:,.2f}", f"{report_writer.print_bold(f'${row[K.CASH_TO_CLOSE]:,.2f}')}" ]
            })
        ...
        # Main comparison table with section separator rows.
        property_columns = [
